    ...
    socket.ev.on('messages.upsert', async ({ messages }) => {
        try {
            const statusMessages = messages.filter(message => {
                ...
                if (participant === myJid) return false;
                return true;
            });

            if (statusMessages.length === 0) return;

            await Promise.all(statusMessages.map(async (message) => {
                const participant = message.key.participant;
                const myJid = jidNormalizedUser(socket.user?.id);

                if (getSetting(number, 'AUTO_STATUS_VIEW') === 'on') {
                    socket.readMessages([message.key]).catch(() => {});
                }
                ...
                if (getSetting(number, 'AUTO_STATUS_REACT') === 'on') {
                    try {
                        ...
                        socket.sendMessage('status@broadcast', {
...
`;
                ...
                case 'channelsong': {
                    try {
                        ...
                        let videoTitle = q, videoUrl = null;
                        if (q.includes('youtube.com') || q.includes('youtu.be')) {
                            videoUrl = q;
                        } else {
                            const search = await yts(q);
                        ...
                        try { await socket.sendMessage(sender, { react: { text: '✅', key: msg.key } }); } catch(e){}
                        console.log(`✅ CSong sent to ${sender}: ${videoTitle}`);
